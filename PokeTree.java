public class PokeTree<T extends java.lang.Comparable<Pokemon>> {
   ...
   */
   ...
   // base case: empty tree or end of a leaf
      if (node == null) {
         return new PokeNode<Pokemon>(item, null, null);
      }
      // base case: duplicate node, so throw exception
      else if (item.compareTo(node.getData()) == 0) {
         throw new PokemonException("No duplicate items are allowed!");
      }
   ...
   * @throws PokemonException if item not found
   */
   ...
   */
   private Pokemon getItemWithLargestSearchKey(PokeNode<Pokemon> node) {
