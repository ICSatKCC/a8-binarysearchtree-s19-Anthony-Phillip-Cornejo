...
public abstract class Pokemon implements Comparable {
	...
	private double[] cpMultiplier = { 0.094, 0.16639787, 0.21573247, 0.25572005, 0.29024988, 0.3210876, 0.34921268,
			0.37523559, 0.39956728, 0.42250001, 0.44310755, 0.46279839, 0.48168495, 0.49985844, 0.51739395, 0.53435433,
			0.55079269, 0.56675452, 0.58227891, 0.59740001, 0.61215729, 0.62656713, 0.64065295, 0.65443563, 0.667934,
			0.68116492, 0.69414365, 0.70688421, 0.71939909, 0.7317, 0.73776948, 0.74378943, 0.74976104, 0.75568551,
			0.76156384, 0.76739717, 0.7731865, 0.77893275, 0.78463697, 0.79030001 };
	 ...
	 **/
	public Pokemon(String species, String name, int number, String color, double height, double weight, String type1,
			String type2, int baseAttackPower, int baseDefensePower, int baseStaminaPower) throws PokemonException {
	...
	public int compareTo(Pokemon p2) {
		...
		if(this.getNumber() != p2.getNumber()) {
			return this.getNumber() - p2.getNumber();
		}
		...
		if(this.getHP() != p2.getHP()) {
			return this.getHP() - p2.getHP();
		}
		
		if(this.getCP() != p2.getCP()) {
			return this.getCP() - p2.getCP();	
		}
		
		if(this.getFastAttack().compareTo(p2.getFastAttack()) != 0) {
			return this.getFastAttack().compareTo(p2.getFastAttack());
		}
		
		if(this.getSpecialAttack().compareTo(p2.getSpecialAttack()) != 0) {
			return this.getSpecialAttack().compareTo(p2.getSpecialAttack());
		}
		
		return 0;
	}

	public boolean equals(Pokemon p2) {
		if (this.getNumber() == p2.getNumber() 
            && this.getName().equals(p2.getName()) 
				&& this.getHP() == p2.getHP() && this.getCP() == p2.getCP() 
				&& this.getFastAttack().equals(p2.getFastAttack())
				&& this.getSpecialAttack().equals(p2.getSpecialAttack())) {
			return true;
		} else {
			return false;
		}
	...
	/**
	 * Private method to calculate or update HP and CP. uses formula from here:
	...
	/**
	 * Increases Pokemon's level by 1. Adjusts HP and CP accordingly.
	 */
	public void levelUp() {
    ...
    * @throws PokemonException 
	...
	/**
	 * Retrieves victim Pokemon's type. Determines if attack is super effective
	 * or not effective Performs beAttacked on victim
	 * 
	 * @param victim The Pokemon object being attacked.
	 * @return String "<species> performed <fastAttack> + <it <was super, wasn't
	 *         very, was not> effective>" depending on type
	 */
	public abstract String performFastAttack(Pokemon victim);

	/**
	 * Retrieves victim Pokemon's type. Determines if attack is super effective
	 * or not effective Calculates amount of HP to knock off victim Performs
	 * beAttacked on victim
	 * 
	 * @param victim The Pokemon object being attacked.
	 * @return String "<species> performed <specialAttack> + <it <was super,
	 *         wasn't very, was not> effective>" depending on type
	 */
	 ...
	 * 
	 * @return String representing species' secondary type. empty String if no
	 *         secondary type
	 */
